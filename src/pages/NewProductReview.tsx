          ...
          <Tabs value={activeTab} onValueChange={(value) => { if (value === "items" && !prNumber) { setErrors({ save: "Save the General information before adding PR items." }); return; } setActiveTab(value); }}>
            ...
            <TabsContent value="general" className="mt-3 space-y-3">
              ...
              <Card><CardContent className="p-0">
                ...
                <div className="grid gap-x-3 gap-y-1.5 p-1.5 md:grid-cols-2 xl:grid-cols-4">
              ...
              <Card><CardContent className="p-0"><SectionTitle title="Customer contact" action={<Button variant="outline" size="sm" className="h-6 text-[10px]"><UserPlus />Add Contact</Button>} />
                <div className="grid gap-x-3 gap-y-1.5 p-1.5 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="grid gap-x-3 gap-y-1.5 p-1.5 md:grid-cols-3 xl:grid-cols-5">
...
function SectionTitle({ title, action }: { title: string; action?: React.ReactNode }) { return <div className="flex min-h-7 items-center justify-between border-b bg-muted/30 px-2.5 py-1"><h2 className="text-[11px] font-semibold">{title}</h2>{action}</div>; }
function Field({ label, value, onChange, error, required, suffix }: { label: string; value: string; onChange: (value: string) => void; error?: string; required?: boolean; suffix?: React.ReactNode }) { return <div className="space-y-0.5"><Label className="text-[8px] uppercase leading-none tracking-wide text-muted-foreground">{label}{required && <span className="text-destructive"> *</span>}</Label><div className="flex gap-1"><Input aria-label={label} value={value} onChange={(event) => onChange(event.target.value)} className={cn("h-6 px-1.5 text-[10px]", error && "border-destructive")} />{suffix}</div>{error && <ErrorText text={error} />}</div>; }
function SelectField({ label, value, options, onChange, error, required }: { label: string; value: string; options: string[]; onChange: (value: string) => void; error?: string; required?: boolean }) { return <div className="space-y-0.5"><Label className="text-[8px] uppercase leading-none tracking-wide text-muted-foreground">{label}{required && <span className="text-destructive"> *</span>}</Label><Select value={value || undefined} onValueChange={onChange}><SelectTrigger aria-label={label} className={cn("h-6 px-1.5 text-[10px]", error && "border-destructive")}><SelectValue placeholder="Select..." /></SelectTrigger><SelectContent>{options.map((option) => <SelectItem key={option} value={option} className="text-[11px]">{option}</SelectItem>)}</SelectContent></Select>{error && <ErrorText text={error} />}</div>; }
