...
const MATCHES = [
  { manufacturer: "FLUKE", model: "789", description: "PROCESSMETER", prItem: "PR05524-002", prStatus: "Lab Management" },
  { manufacturer: "FLUKE", model: "789", description: "MULTIFUNCTION PROCESS CALIBRATOR", prItem: "PR05701-001", prStatus: "Approved" },
  { manufacturer: "FLUKE", model: "789-12", description: "PROCESSMETER", prItem: "PR06961-001", prStatus: "Initiator" },
];
    ...
    <div className="flex h-dvh flex-col bg-background">
      ...
      <Dialog open={wizardOpen} onOpenChange={(open) => { setWizardOpen(open); if (!open) setErrors({}); }}><DialogContent className="max-h-[90vh] max-w-4xl overflow-y-auto p-0">
        ...
        <div className="min-h-72 p-5">
          ...
          </div>{exactDuplicate && <div className="rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-center text-xs font-semibold text-destructive">The same manufacturer and model already exists in CalMapp. Please check this before you continue.</div>}{matches.length > 0 && <div className="rounded-md border"><Table><TableHeader><TableRow className="bg-muted/40"><TableHead>Manufacturer</TableHead><TableHead>Model</TableHead><TableHead>Description</TableHead><TableHead>PR Item</TableHead><TableHead>PR Status</TableHead></TableRow></TableHeader><TableBody>{matches.map((match) => <TableRow key={`${match.model}-${match.description}`} className="text-xs"><TableCell>{match.manufacturer}</TableCell><TableCell>{match.model}</TableCell><TableCell>{match.description}</TableCell><TableCell><Button variant="link" className="h-auto p-0 text-xs text-foreground underline" onClick={() => setItemDraft((previous) => ({ ...previous, manufacturer: match.manufacturer, model: match.model, description: match.description }))}>{match.prItem}</Button></TableCell><TableCell><span className="inline-flex items-center gap-1 rounded-full bg-muted px-2 py-0.5 text-[10px]"><span className="h-1.5 w-1.5 rounded-full bg-info" />{match.prStatus}</span></TableCell></TableRow>)}</TableBody></Table></div>}</div>}
