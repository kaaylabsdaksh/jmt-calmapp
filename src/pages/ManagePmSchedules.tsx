...
import { ArrowLeft, CalendarClock, ChevronDown, ChevronRight, CircleAlert, ClipboardList, Layers3, Plus, RotateCcw, Search } from "lucide-react";
...
const ManagePmSchedules = () => {
  ...
  const summary = useMemo(() => ({
    ...
    active: filtered.filter((row) => row.scheduleStatus === "Active").length,
    pending: filtered.filter((row) => row.scheduleStatus === "Pending").length,
    attention: filtered.filter((row) => row.scheduleStatus === "Cancelled" || row.lastResult === "Not Performed").length,
  }), [filtered]);
  ...
  return <div className="flex h-dvh min-h-0 flex-col bg-background">
    ...
    <main className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-3 py-4 sm:px-4 lg:px-6">
      {editing ? <ScheduleEditor schedule={editing} isNew={isNew} onChange={updateEditing} onSave={saveSchedule} onLifecycle={setLifecycle} onBack={() => navigate(LIST_PATH)} /> : <>
        <section className="shrink-0 overflow-hidden border bg-card shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-muted/20 px-4 py-3"><div className="flex items-center gap-3"><span className="flex h-8 w-8 items-center justify-center rounded-md border bg-card text-muted-foreground"><Search className="h-4 w-4" /></span><div><h2 className="text-sm font-semibold">Search Criteria</h2><p className="text-[11px] text-muted-foreground">Find preventive maintenance and interim-check schedules.</p></div></div><Button size="sm" className="h-7 gap-1.5 text-[11px]" onClick={() => navigate(`${LIST_PATH}/new`)}><Plus className="h-3.5 w-3.5" /> Add New Schedule</Button></div>
          <div className="divide-y">
            <FilterBand title="Schedule" subtitle="Type, description, status and frequency" className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
            ...
            <FilterBand title="Assignment" subtitle="Assigned station and linked standard" className="grid gap-3 md:grid-cols-2">
            ...
            <FilterBand title="Date Windows" subtitle="Filter due and terminal date ranges" className="grid gap-3 md:grid-cols-2">
        ...
        <section className="flex shrink-0 flex-col overflow-hidden border bg-card shadow-sm">
          <div className="grid border-b bg-muted/20 sm:grid-cols-2 xl:grid-cols-4"><SummaryTile icon={ClipboardList} label="Matching schedules" value={summary.total} /><SummaryTile icon={CalendarClock} label="Active" value={summary.active} tone="success" /><SummaryTile icon={Layers3} label="Pending" value={summary.pending} tone="warning" /><SummaryTile icon={CircleAlert} label="Needs attention" value={summary.attention} tone="destructive" /></div>
          <div className="flex items-center justify-between border-b px-4 py-3"><div><h2 className="text-sm font-semibold">Schedule Results</h2><p className="text-[11px] text-muted-foreground">Expand a row to review completion history.</p></div><span className="text-[10px] font-medium uppercase text-muted-foreground">Sorted by due date</span></div>
...
const ScheduleEditor = ({ schedule, isNew, onChange, onSave, onLifecycle, onBack }: { schedule: ManagedSchedule; isNew: boolean; onChange: <K extends keyof ManagedSchedule>(key: K, value: ManagedSchedule[K]) => void; onSave: () => void; onLifecycle: (status: ScheduleState) => void; onBack: () => void }) => {
  ...
  return <section className="flex min-h-0 flex-1 flex-col overflow-hidden border bg-card shadow-sm">
    <div className="flex-1 overflow-auto">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b bg-muted/20 px-5 py-3"><div className="flex items-center gap-3"><span className="flex h-9 w-9 items-center justify-center rounded-md border bg-card text-muted-foreground"><CalendarClock className="h-4 w-4" /></span><div><h2 className="text-sm font-semibold">{isNew ? "Add New Schedule" : schedule.templateDescription}</h2><p className="text-[11px] text-muted-foreground">{isNew ? "Create a preventive maintenance or interim-check schedule." : `${schedule.id} · ${schedule.station}`}</p></div></div><div className="flex items-center gap-2"><span className="text-[10px] text-muted-foreground">{isNew ? "ID assigned on save" : schedule.id}</span><StatusBadge status={schedule.scheduleStatus} /></div></div>

      <EditorBand number="01" title="Schedule Overview" description="Define the schedule type and purpose.">
        <div className="grid gap-3 md:grid-cols-[minmax(180px,0.5fr)_minmax(0,1.5fr)]"><CompactField label="Type *"><Select disabled={readOnly} value={schedule.type} onValueChange={(value) => onChange("type", value as ManagedSchedule["type"])}><SelectTrigger className={CONTROL}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="IM">Interim Check</SelectItem><SelectItem value="PM">Preventive Maintenance</SelectItem></SelectContent></Select></CompactField><CompactField label="Description *"><Input disabled={readOnly} className={CONTROL} value={schedule.templateDescription} onChange={(event) => onChange("templateDescription", event.target.value)} /></CompactField></div>
      </EditorBand>

      <EditorBand number="02" title="Template & Station Assignment" description="Connect the schedule to its controlling document and station.">
        <div className="grid gap-3 md:grid-cols-2"><CompactField label="Template *"><Select disabled={readOnly} value={schedule.documentTool || undefined} onValueChange={(value) => { const template = PM_TEMPLATE_RECORDS.find((item) => item.document === value); onChange("documentTool", value); onChange("templateDescription", template?.description ?? ""); }}><SelectTrigger className={CONTROL}><SelectValue placeholder="Select template" /></SelectTrigger><SelectContent>{PM_TEMPLATE_RECORDS.map((template) => <SelectItem key={template.id} value={template.document}>{template.document}</SelectItem>)}</SelectContent></Select></CompactField><CompactField label="Station *"><Select disabled={readOnly} value={schedule.station || undefined} onValueChange={(value) => { const standard = STANDARDS.find((item) => value.includes(item.standardNo)); onChange("station", value); onChange("standards", standard ? [standard.standardNo] : schedule.standards); }}><SelectTrigger className={CONTROL}><SelectValue placeholder="Select station" /></SelectTrigger><SelectContent>{PM_STATIONS.map((station) => <SelectItem key={station} value={station}>{station}</SelectItem>)}</SelectContent></Select></CompactField></div>{selectedTemplate && <button type="button" className="mt-2 text-[11px] font-medium text-info hover:underline" onClick={() => toast({ title: selectedTemplate.document, description: "Document preview is not available in mock data." })}>View Doc / Tool: {selectedTemplate.document}</button>}
      </EditorBand>

      <EditorBand number="03" title="Frequency & Dates" description="Set the repeat interval and active scheduling window.">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><CompactField label="Frequency *"><Select disabled={readOnly} value={schedule.frequency} onValueChange={(value) => onChange("frequency", value)}><SelectTrigger className={CONTROL}><SelectValue /></SelectTrigger><SelectContent><SelectItem value="D">Daily</SelectItem><SelectItem value="M">Monthly</SelectItem></SelectContent></Select></CompactField><CompactField label="Interval"><Select disabled={readOnly} value={schedule.interval} onValueChange={(value) => onChange("interval", value)}><SelectTrigger className={CONTROL}><SelectValue /></SelectTrigger><SelectContent>{["1", "3", "6", "12"].map((value) => <SelectItem key={value} value={value}>{value}</SelectItem>)}</SelectContent></Select></CompactField><CompactField label="Year"><Select disabled={readOnly} value={schedule.year} onValueChange={(value) => onChange("year", value)}><SelectTrigger className={CONTROL}><SelectValue /></SelectTrigger><SelectContent>{["2026", "2027", "2028"].map((year) => <SelectItem key={year} value={year}>{year}</SelectItem>)}</SelectContent></Select></CompactField><CompactField label="Frequency Due Date *"><ModernDatePicker disabled={readOnly} size="sm" value={schedule.dueDate} onChange={(value) => onChange("dueDate", dateText(value))} /></CompactField><CompactField label="Terminal Date *"><ModernDatePicker disabled={readOnly} size="sm" value={schedule.terminalDate} onChange={(value) => onChange("terminalDate", dateText(value))} /></CompactField></div>
      </EditorBand>

      <EditorBand number="04" title="Linked Standards" description="Standards are assigned automatically from the selected station.">
        <div className="overflow-hidden border"><Table><TableHeader><TableRow><TableHead className="h-7 text-[10px]">Std #</TableHead><TableHead className="h-7 text-[10px]">Manufacturer</TableHead><TableHead className="h-7 text-[10px]">Model</TableHead></TableRow></TableHeader><TableBody>{standardRows.length ? standardRows.map((standard) => standard && <TableRow key={standard.id}><TableCell className="py-1.5 font-semibold text-info">{standard.standardNo}</TableCell><TableCell className="py-1.5">{standard.manufacturer}</TableCell><TableCell className="py-1.5">{standard.model}</TableCell></TableRow>) : <TableRow><TableCell colSpan={3} className="h-14 text-center text-[11px] text-muted-foreground">Select a station to view linked standards.</TableCell></TableRow>}</TableBody></Table></div>
      </EditorBand>

      <EditorBand number="05" title="Audit Information" description="Schedule ownership and most recent update.">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4"><Audit label="Created By" value={schedule.createdBy} /><Audit label="Created Date" value={schedule.createdDate || "On save"} /><Audit label="Modified By" value={schedule.modifiedBy || "—"} /><Audit label="Modified Date" value={schedule.modifiedDate || "—"} /></div>
      </EditorBand>

      {!isNew && <><EditorBand number="06" title="History" description={`${schedule.histories.length} completed schedule entries.`}><HistoryTable schedule={schedule} /></EditorBand><EditorBand number="07" title="Comments" description="Notes shared with the work-order record."><WorkOrderItemComments workOrderItemId={schedule.id} /></EditorBand></>}
    </div>
    <div className="shrink-0 flex items-center justify-between gap-2 border-t bg-card px-4 py-3 shadow-[0_-2px_8px_hsl(var(--foreground)/0.06)]"><Button variant="outline" className="h-8 gap-1.5 text-xs" onClick={onBack}><ArrowLeft className="h-3.5 w-3.5" /> Back</Button><div className="flex items-center gap-2">{!isNew && schedule.scheduleStatus === "Pending" && <Button variant="outline" className="h-8 text-xs" onClick={() => onLifecycle("Active")}>Activate Schedule</Button>}{!isNew && (schedule.scheduleStatus === "Pending" || schedule.scheduleStatus === "Active") && <Button variant="destructive" className="h-8 text-xs" onClick={() => onLifecycle("Cancelled")}>Cancel Schedule</Button>}<Button className="h-8 text-xs" disabled={readOnly} onClick={onSave}>{isNew ? "Create Schedule" : "Save Changes"}</Button></div></div>
  </section>;
};
...
const FilterBand = ({ title, subtitle, className, children }: { title: string; subtitle: string; className?: string; children: React.ReactNode }) => <div className="grid gap-3 px-4 py-3 xl:grid-cols-[150px_minmax(0,1fr)]"><div><p className="text-[10px] font-bold uppercase text-foreground/80">{title}</p><p className="mt-0.5 text-[10px] leading-4 text-muted-foreground">{subtitle}</p></div><div className={className}>{children}</div></div>;
const SummaryTile = ({ icon: Icon, label, value, tone = "default" }: { icon: React.ElementType; label: string; value: number; tone?: "default" | "success" | "warning" | "destructive" }) => <div className="flex items-center gap-3 border-b px-4 py-3 last:border-b-0 sm:border-r sm:odd:border-r sm:last:border-r-0 xl:border-b-0"><span className={cn("flex h-8 w-8 items-center justify-center rounded-md border bg-card text-muted-foreground", tone === "success" && "text-success", tone === "warning" && "text-warning-foreground", tone === "destructive" && "text-destructive")}><Icon className="h-4 w-4" /></span><div><p className="text-lg font-semibold leading-none tabular-nums">{value}</p><p className="mt-1 text-[10px] uppercase text-muted-foreground">{label}</p></div></div>;
const EditorBand = ({ number, title, description, children }: { number: string; title: string; description: string; children: React.ReactNode }) => <section className="grid border-b lg:grid-cols-[220px_minmax(0,1fr)]"><div className="bg-muted/15 px-5 py-4"><div className="flex items-start gap-3"><span className="text-[10px] font-semibold text-muted-foreground">{number}</span><div><h3 className="text-xs font-semibold">{title}</h3><p className="mt-1 max-w-48 text-[10px] leading-4 text-muted-foreground">{description}</p></div></div></div><div className="px-5 py-4">{children}</div></section>;
const Audit = ({ label, value }: { label: string; value: string }) => <div className="border-l-2 border-muted pl-3"><p className="text-[10px] uppercase text-muted-foreground">{label}</p><p className="mt-0.5 text-[11px] font-medium">{value}</p></div>;
