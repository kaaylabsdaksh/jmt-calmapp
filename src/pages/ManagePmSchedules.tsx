      ...
      <Accordion type="multiple" defaultValue={["assignment", "settings", ...(isNew ? [] : ["history", "comments"] as string[])]} className="space-y-3 px-4 py-4">
        <AccordionItem value="assignment" className="border bg-card">
          ...
          <AccordionContent className="px-4 pb-4 pt-0">
            <div className="space-y-4">
              <CompactField label="Template *"><Select disabled={readOnly} value={schedule.documentTool || undefined} onValueChange={(value) => { const template = PM_TEMPLATE_RECORDS.find((item) => item.document === value); onChange("documentTool", value); onChange("templateDescription", template?.description ?? ""); }}><SelectTrigger className={CONTROL}><SelectValue placeholder="Select template" /></SelectTrigger><SelectContent>{PM_TEMPLATE_RECORDS.map((template) => <SelectItem key={template.id} value={template.document}>{template.document}</SelectItem>)}</SelectContent></Select></CompactField>
              <CompactField label="Station *"><Select disabled={readOnly} value={schedule.station || undefined} onValueChange={(value) => { const standard = STANDARDS.find((item) => value.includes(item.standardNo)); onChange("station", value); onChange("standards", standard ? [standard.standardNo] : schedule.standards); }}><SelectTrigger className={CONTROL}><SelectValue placeholder="Select station" /></SelectTrigger><SelectContent>{PM_STATIONS.map((station) => <SelectItem key={station} value={station}>{station}</SelectItem>)}</SelectContent></Select></CompactField>
              {selectedTemplate && <button type="button" className="text-[11px] font-medium text-info hover:underline" onClick={() => toast({ title: selectedTemplate.document, description: "Document preview is not available in mock data." })}>Doc / Tool: {selectedTemplate.document}</button>}
              <div className="overflow-hidden border"><Table><TableHeader><TableRow><TableHead className="h-7 text-[10px]">Std #</TableHead><TableHead className="h-7 text-[10px]">Manufacturer</TableHead><TableHead className="h-7 text-[10px]">Model</TableHead></TableRow></TableHeader><TableBody>{standardRows.length ? standardRows.map((standard) => standard && <TableRow key={standard.id}><TableCell className="py-1.5">{standard.standardNo}</TableCell><TableCell className="py-1.5">{standard.manufacturer}</TableCell><TableCell className="py-1.5">{standard.model}</TableCell></TableRow>) : <TableRow><TableCell colSpan={3} className="h-12 text-center text-[11px] text-muted-foreground">Select a station to view linked standards.</TableCell></TableRow>}</TableBody></Table></div>
            </div>
          </AccordionContent>
        </AccordionItem>
        ...
        {!isNew && <>
          ...
          <AccordionItem value="comments" className="border bg-card">
            ...
            <AccordionContent className="px-4 pb-4 pt-0"><WorkOrderItemComments workOrderItemId={schedule.id} /></AccordionContent>
          </AccordionItem>
        </>}
      </Accordion>
    </div>
