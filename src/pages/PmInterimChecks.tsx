  ...
  return (
    <TooltipProvider>
      <div className="min-h-full bg-background">
        ...
        <ManagerDialog kind={manager} onClose={() => setManager(null)} />
      </div>
    </TooltipProvider>
  );
};
...
const ManagerDialog = ({ kind, onClose }: { kind: ManagerKind; onClose: () => void }) => {
  ...
  const items = kind === "stations" ? PM_STATIONS.slice(0, 8) : kind === "templates" ? PM_TEMPLATES : PM_SCHEDULES.slice(0, 8).map((row) => `#${row.id} · ${row.station}`);
  return <Dialog open={kind !== null} onOpenChange={(open) => !open && onClose()}><DialogContent className="max-w-2xl"><DialogHeader><DialogTitle className="flex items-center gap-2 text-base"><Wrench className="h-4 w-4" />{title}</DialogTitle><DialogDescription>Review the records used by PM and interim-check schedules.</DialogDescription></DialogHeader><div className="max-h-[50vh] overflow-auto rounded-md border">{items.map((item, index) => <div key={item} className="flex items-center justify-between border-b px-3 py-2 last:border-b-0"><div><p className="text-xs font-medium">{item}</p><p className="text-[10px] text-muted-foreground">{kind === "schedules" ? "Active schedule" : `${index + 1} linked schedule${index === 0 ? "" : "s"}`}</p></div><Button variant="ghost" size="sm" className="h-7 text-[11px]" onClick={() => toast({ title: `${title.slice(7, -1) || "Record"} selected`, description: item })}>View</Button></div>)}</div><DialogFooter><Button variant="outline" className="h-8 text-xs" onClick={onClose}>Close</Button><Button className="h-8 text-xs" onClick={() => toast({ title: `New ${kind === "stations" ? "station" : kind === "templates" ? "template" : "schedule"} started` })}>Add New</Button></DialogFooter></DialogContent></Dialog>;
};
