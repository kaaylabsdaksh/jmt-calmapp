...
import { buildTemplateSchedules, PM_TEMPLATE_RECORDS, PmTemplateRecord, PmTemplateStatus } from "@/lib/standards/pm-templates";
...
const TemplateEditor = ({ template, isNew, pendingFile, fileInput, onFile, onChange, onCancel, onSave }: { template: PmTemplateRecord; isNew: boolean; pendingFile: string; fileInput: React.RefObject<HTMLInputElement>; onFile: (value: string) => void; onChange: <K extends keyof PmTemplateRecord>(key: K, value: PmTemplateRecord[K]) => void; onCancel: () => void; onSave: () => void }) => <section className="flex min-h-0 flex-1 flex-col bg-card">
