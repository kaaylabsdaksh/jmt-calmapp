...
const CDR_STATUSES = ["Created", "Under Review", "Pending Approval", "Completed"];
const CUSTOMERS = [
...
const MOCK_CRS: CrRecord[] = Array.from({ length: 26 }, (_, i) => ({
  id: `cr-${i + 1}`,
  ...
  cdrStatus: CDR_STATUSES[i % CDR_STATUSES.length],
  ...
  documents: [
    { name: "master-service-agreement.pdf", version: "v2.1", uploaded: "03/14/2026" },
    { name: "pricing-exhibit-a.xlsx", version: "v1.4", uploaded: "03/18/2026" },
    { name: "insurance-certificate.pdf", version: "v1.0", uploaded: "03/21/2026" },
  ].slice(0, (i % 3) + 1),
  notes:
    i % 2 === 0
      ? "Legal requested revision to indemnity clause before approval routing."
      : "Customer returned redlines on payment terms; pending reviewer confirmation.",
}));
...
const ContractReviews = () => {
  ...
  const rows = useMemo(() => {
    ...
    const filtered = MOCK_CRS.filter((r) => {
      ...
      if (q) {
        const hay = [r.cr, r.customer, r.account, r.docType, r.cdr, r.createdBy, r.submittedBy]
          .join(" ")
          .toLowerCase();
  ...
  const kpis = useMemo(
    () => [
      ...
      { label: "Rejected", value: rows.filter((r) => r.status === "rejected").length, icon: XCircle },
      {
        ...
        value: rows.filter((r) => r.status === "waiting-approval" || r.status === "pending-review").length,
        icon: Clock,
      },
    ],
    [rows]
  ...
  const runSearch = () => {
    ...
    window.setTimeout(() => {
      setLoading(false);
      toast({ title: "Search Complete", description: `${rows.length} contract reviews found.` });
    }, 600);
  };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => notify("Contract Review Created", "A new contract review was started.")}
            >
            ...
            <Button
              ...
              size="sm"
              className="h-8 text-xs"
              onClick={() => notify("Export Completed", "Current results exported.")}
            >
            ...
            <Collapsible open={advancedOpen} onOpenChange={setAdvancedOpen} className="space-y-2">
              <div className="flex flex-wrap items-center gap-2">
                <div className="relative min-w-[200px] flex-1">
                  ...
                  <Input
                    ...
                    placeholder="Search CR #, customer, account, CDR #, reviewer"
              ...
              <CollapsibleContent className="space-y-2">
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 pt-1">
                  ...
                  <div className="space-y-1">
                    <label className={labelCls}>Linked CDR #</label>
                ...
                <Table>
                  <TableHeader className="sticky top-0 z-10 bg-card">
                    <TableRow className="[&>th]:py-1.5 [&>th]:text-[10px] [&>th]:uppercase [&>th]:tracking-wide [&>th]:whitespace-nowrap">
                      ...
                      {isVisible("cdr") && <TableHead>Linked CDR #</TableHead>}
                  ...
                  <TableBody>
                    {rows.map((r) => (
                      <TableRow key={r.id} className="[&>td]:py-1.5 [&>td]:text-xs [&>td]:whitespace-nowrap">
                        ...
                        {isVisible("cdr") && (
                          <TableCell>
                            <span className="text-foreground underline underline-offset-2">{r.cdr.split("-").pop()}</span>
                          </TableCell>
                        )}
                        {isVisible("cdrStatus") && <TableCell className="text-muted-foreground">{r.cdrStatus}</TableCell>}
                        ...
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              <DropdownMenuItem className="text-xs" onClick={() => notify("CDR Linked", `${r.cr} → ${r.cdr}`)}>
                              ...
                              <DropdownMenuItem className="text-xs" onClick={() => notify("Download Started", r.cr)}>
                                <Download className="h-3.5 w-3.5 mr-2" />Download Contract
                              </DropdownMenuItem>
                              <DropdownMenuItem className="text-xs" onClick={() => notify("Export Completed", r.cr)}>
              ...
              <div className="mt-4 space-y-4">
                <section className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs items-center">
                    <div><span className="text-muted-foreground">CR #:</span> {drawer.cr}</div>
                    ...
                    <div className="col-span-2"><span className="text-muted-foreground">Customer:</span> {drawer.customer}</div>
                  </div>
                </section>
                <Separator />
                <section className="space-y-1.5">
                  ...
                  <div className="grid grid-cols-2 gap-2 text-xs">
                    <div><span className="text-muted-foreground">CDR #:</span> {drawer.cdr}</div>
