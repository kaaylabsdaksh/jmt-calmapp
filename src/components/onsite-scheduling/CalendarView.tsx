...
const NON_SERVICE_STYLES: Record<NonServiceEntry['type'], string> = {
  PTO: 'bg-purple-400 text-white border-purple-500 border-dashed',
  Travel: 'bg-cyan-500 text-white border-cyan-600 border-dashed',
  'Out of Service': 'bg-neutral-500 text-white border-neutral-600 border-dashed',
  Tentative: 'bg-yellow-200 text-yellow-900 border-yellow-400 border-dashed',
  ...
  // mis-tagged entry.
  Other: 'bg-slate-400 text-white border-slate-500 border-dashed',
};
...
const CalendarView: React.FC = () => {
  ...
  return (
    <div className="flex flex-col gap-3 p-4">
      {/* Toolbar */}
      <div className="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-white p-3 shadow-sm dark:bg-background">
      ...
      <div
        id="decision-D9"
        className={cn(
          'flex flex-col gap-2 rounded-md border bg-white p-2.5 text-[11px] shadow-sm transition-shadow dark:bg-background',
          highlightedAnchorId === 'decision-D9' && 'ring-2 ring-amber-400'
        )}
      >
        <div className="flex items-center gap-1.5 text-muted-foreground">
          <span className="font-medium">
            Event-type / Readiness / Status color legend
          </span>
        ...
        <div className="flex flex-wrap gap-3">
          ...
          {NON_SERVICE_ENTRY_TYPES.map((t) => (
            <span key={t} className="flex items-center gap-1.5">
              <span
                className={cn('h-2.5 w-4 rounded-sm border-2', NON_SERVICE_STYLES[t])}
              />
              {t}
            </span>
          ))}
