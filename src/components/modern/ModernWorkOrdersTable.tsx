  ...
  const getBatchItems = (batchId: string) => {
    ...
    const itemSets: Record<string, Array<{ id: string; item: string; location: string; itemCreated: string; action: string; itemStatus: string; manufacturer: string; model: string; description: string; serialNumber: string; deliverByDate: string; followUpDate: string }>> = {
      "383727": [
        ...
        { id: "002", item: "Safety Helmet", location: "Houston", itemCreated: "10/15/2021", action: "CALIBRATE", itemStatus: "Back to Customer", manufacturer: "SafetyFirst", model: "SF-100", description: "Industrial Safety Helmet with Face Shield", serialNumber: "SF123456", deliverByDate: "11/15/2021", followUpDate: "11/28/2024" },
      ...
      "390118": [
        ...
        { id: "002", item: "Relay", location: "Lab B", itemCreated: "12/10/2022", action: "TEST", itemStatus: "Back to Customer", manufacturer: "Siemens", model: "S7-200", description: "Protection Relay", serialNumber: "SIE998877", deliverByDate: "01/05/2023", followUpDate: "08/11/2023" },
      ],
    };
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200">
      ...
      <div className="overflow-hidden">
        {viewMode === 'list' ? (
                          ...
                          {col.type === 'date' ? (
                            ...
                            <div className="relative flex-1">
                              ...
                              <Input
                                ...
                                className={cn("h-7 text-[11px] pr-6 border-muted bg-muted/30 rounded-md placeholder:text-muted-foreground/40 focus:bg-background focus:border-primary/30 transition-colors", searchViewMode === 'csa' ? "pl-2.5" : "pl-6")}
                              />
                          ...
                          {col.type === 'date' ? (
                            ...
                            <>
                              <Input
                                ...
                                value={columnFilters[col.key] || ''}
                                onChange={(e) => setColumnFilters(prev => ({ ...prev, [col.key]: e.target.value }))}
                                className="h-6 text-[11px] text-foreground pl-1.5 pr-5 border-muted bg-muted/30 rounded-md placeholder:text-muted-foreground/40 focus:bg-background focus:border-primary/30 transition-colors"
                              />
                              {columnFilters[col.key] && (
                                <button
                                  onClick={() => setColumnFilters(prev => ({ ...prev, [col.key]: '' }))}
          ...
          // Grid View - Cards
          <div className={cn("grid gap-3", searchViewMode === 'csa' ? "p-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3" : "p-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6")}>
            {!hasSearched ? (
              ...
              <>
                ...
                <div 
                  ...
                  className={cn(
                    ...
                    searchViewMode === 'csa'
                      ? "bg-gradient-to-br from-white to-slate-50 border border-slate-200 hover:shadow-lg hover:border-slate-300 hover:-translate-y-0.5"
        ...
        <div className="p-6 border-t border-gray-200 flex justify-between items-center">
          <div className="flex items-center gap-4">
            <span className="text-sm text-gray-600">
              Showing {((currentPage - 1) * itemsPerPage) + 1}-{Math.min(currentPage * itemsPerPage, currentView === 'batch' ? sortedBatches.length : sortedFilteredItems.length)} of {currentView === 'batch' ? sortedBatches.length : sortedFilteredItems.length} items
            </span>
            <div className="flex items-center gap-2">
